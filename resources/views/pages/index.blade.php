				...
				<div class="col-lg-3 mb-5 mt-5 product">
					<div class="single-item">
						...
						<div class="single-item-header">
							<a href="{{ route('page.detailproduct',['slug'=>$sale->slug,'id'=> $sale->id ]) }}"><img width="270px" height="200px" src="upload/product/{{ $sale->image }}" alt=""></a>
						</div>
						<div class="single-item-body">
							<h4 class="single-item-title mt-3" style="color: #030303">{{ $sale->name }}</h4>
						...
						{{-- <div class="single-item-caption">
							<a class="add-to-cart pull-left" href="{{ route('page.detailproduct',['slug'=>$sale->slug,'id'=> $sale->id ]) }}"><i class="fa fa-shopping-cart" aria-hidden="true" style="font-size: 50px; margin-left: 5px"></i></a>
							<a style="background-color: #3A5C83; font-size: 18px; margin-left: 18px;" class="btn btn-success" href="{{ route('page.detailproduct',['slug'=>$sale->slug,'id'=> $sale->id ]) }}"> Details <i style="font-size: 20px; margin-left: 10px;margin-top:6px" class="fa fa-angle-double-right" aria-hidden="true"></i></a>
			...
			<div class="row">
				...
				<div class="col-lg-3 mb-5 mt-5 product">
					<div class="single-item">
						...
						<div class="ribbon-wrapper"><div class="ribbon sale">Sale</div></div>
						<div class="single-item-header">
							<a href="{{ route('page.detailproduct',['slug'=>$sell->slug,'id'=> $sell->id ]) }}"><img width="270px" height="200px" src="upload/product/{{ $sell->image }}" alt=""></a>
						</div>
						<div class="single-item-body">
							<h4 class="single-item-title mt-3" style="color: #030303">{{ $sell->name }}</h4>
							<p class="single-item-price">
								@if($sell->promotion_price != 0)
								<span class="flash-del">{{ number_format($sell->unit_price ,0 ,'.' ,'.').' Đ' }}</span>

								<span class="flash-sale">{{ number_format($sell->promotion_price ,0 ,'.' ,'.').' Đ' }}</span>
								@else
										{{-- <span class="flash-sale">{{ $product->unit_price }}đ</span> --}}
										<span class="flash-sale">{{ number_format($sell->unit_price ,0 ,'.' ,'.').' Đ' }}</span>
	
								@endif
							</p>
						</div>
						{{-- <div class="single-item-caption">
							<a class="add-to-cart pull-left" href="{{ route('page.detailproduct',['slug'=>$sell->slug,'id'=> $sell->id ]) }}"><i class="fa fa-shopping-cart" aria-hidden="true" style="font-size: 50px; margin-left: 5px"></i></a>
							<a style="background-color: #3A5C83; font-size: 18px; margin-left: 18px;" class="btn btn-success" href="{{ route('page.detailproduct',['slug'=>$sell->slug,'id'=> $sell->id ]) }}"> Details <i style="font-size: 20px; margin-left: 10px;margin-top:6px" class="fa fa-angle-double-right" aria-hidden="true"></i></a>
							<div class="clearfix"></div>
						</div> --}}
